...
public abstract class ProcessButton extends FlatButton {
	...
	private void initAttributes(Context context, AttributeSet attributeSet) {
		...
		try {
			mLoadingText = attr
					.getString(R.styleable.ProcessButton_progressText);
			mCompleteText = attr
					.getString(R.styleable.ProcessButton_completeText);
	...
	protected void onLoadingComplete() {
		if (getCompleteText() != null) {
			// setText(getCompleteText());//自己控制显示
		}

		if (getCompleteDrawable() != null) {
			//setBackground(getCompleteDrawable());//不能在ListView 里面调用视图的绘画操作,这是ListView 一个特性产生的BUG
		}
	}
